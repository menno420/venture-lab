...
The registered skill set for venture-lab: every recurring action that
...
|---|---|---|---|
| `session-close` | Land the session — claim, born-red card first, READY PR, batched work, close-out docs, flip complete last, then merge (directly or via the enabler). | `read`, `edit`, `run` | `python3 bootstrap.py check --strict`<br>`python3 bootstrap.py check --strict` |
