...
|---|---|---|---|
| `session-close` | Land the session — claim, born-red card first, READY PR, batched work, close-out docs, flip complete last; never self-merge. | `read`, `edit`, `run` | `python3 bootstrap.py check --strict`<br>`python3 bootstrap.py check --strict` |
