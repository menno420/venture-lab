...
- `autonomous-project` · **overview panel (ORDER 015): seat-side unverifiable; owner-side feature** — no tool or surface resembling an overview/panel/dashboard is exposed to this seat…
- `any` · merging own green PRs is normal agent work — agents merge their own/sibling green PRs directly (MCP/REST), flip draft→ready, arm auto-merge; NOT owner-gated. The enabler also lands `claude/*` heads on green. (Old "self-merge classifier-denied → agent-unlandable" note was a false standing wall — superseded.)…
