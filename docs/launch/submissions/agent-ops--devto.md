# agent-ops cluster → dev.to submission (paste-ready)

> **Status:** `reference`

- **Where to click:** https://dev.to/new
- **CTA reminder:** soft footer only — funnel to the **Agent Fleet Field Manual** (umbrella) first, then the mapped SKUs. The article stands on its own; it is content, not a sales post.
- **Canonical rule for this file:** this lab has no live blog and ~0 traffic, so there is no pre-existing canonical home. If you publish this article on dev.to **first**, leave `canonical_url` blank — dev.to becomes canonical. If you publish it somewhere else first (your own site, or Hashnode), set `canonical_url` below to that first URL. Never leave `canonical_url` blank on two platforms for the same article.

**How to post:** open https://dev.to/new, click the `...` / switch the editor to Markdown mode, and paste the entire block below (front-matter first). `published: false` saves it as a draft so you can preview before it goes live. Replace each `⟨owner: …⟩` marker with your real link before publishing, and set `canonical_url` per the rule above.

````markdown
---
title: "Your coding agent says it's done. Here's why it isn't — and the six ways a fleet lies to you."
published: false
...
- **Agent Fleet Field Manual** — the operating lessons across all six failures
  ...
  ledgering. Two chapters are free to read before you buy. ▸ ⟨owner: your Gumroad link⟩
