# The D1 Lesson: Never Claim a Payment Path Works Without Executing It

> **Status:** `reference`

*This is a free, standalone chapter from the **Agent Fleet Field Manual** — a cited field manual for running autonomous coding-agent fleets. It is one of two free chapters. If you build agents that touch payments, this is the most expensive lesson the lane learned, offered up front.*
...
This is the lesson the lane calls **D1**: *never claim a payment path works without executing it.*
...
That third part is the discipline. The fixtures are not written from memory — they are vendored, meaning each field name and type was checked against the provider's own SDK source, with recorded provenance.
...
A green test that agrees with its own author's memory is the most dangerous artifact in an agent fleet, because it *looks* exactly like verification.

---

*This is a free chapter from the **Agent Fleet Field Manual** ($39). If it saved you one silently-dropped order, the other nine chapters apply the same "prove it, don't assert it" discipline across the whole operating surface — merge walls, the owner-action money protocol, born-red work tracking, and honest negative ledgering. Every lesson is cited to a real repository artifact.*
