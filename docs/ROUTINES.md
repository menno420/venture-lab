# venture-lab — routines & wake-chain doctrine

> **Status:** `historical`
>
> ⚠️ **DORMANT / HISTORICAL — no routines are armed.** The owner is winding down
> the autonomy apparatus (Claude Code Projects EAP goes read-only 2026-07-21;
> projects are being recreated fresh), and every EAP-era trigger has died with
> its session. Do NOT self-arm, re-arm, or trust any trigger id from this
> doctrine on your own — re-arming is a deliberate owner action post-relaunch.
> Kept as reference for the wake-chain lessons only. Current truth:
