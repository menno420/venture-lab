#!/bin/sh
# Build the buyer bundle for Shopify Webhook Test Kit v0.1.
# Bundles: the harness (shwtk.py + shwtk.js), the example handler, the HTTP-layer
...
# build is byte-reproducible (same convention as
# stripe/github/slack-webhook-test-kit).
set -eu

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
SRC="$SCRIPT_DIR"
BUNDLE_NAME="shopify-webhook-test-kit-v0.1"
DIST_DIR="$SRC/dist"
...
cp "$SRC/shwtk.py"               "$ROOT/shwtk.py"
cp "$SRC/shwtk.js"               "$ROOT/shwtk.js"
