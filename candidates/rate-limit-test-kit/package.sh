#!/bin/sh
# Build the buyer bundle for Rate-Limit Test Kit v0.1.
# Bundles: the harness (rltk.py + rltk.js), the CORRECT + NAIVE reference limiters,
...
# build is byte-reproducible (same convention as the webhook + idempotency kits).
set -eu

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
SRC="$SCRIPT_DIR"
BUNDLE_NAME="rate-limit-test-kit-v0.1"
DIST_DIR="$SRC/dist"
...
cp "$SRC/rltk.py"                  "$ROOT/rltk.py"
cp "$SRC/rltk.js"                  "$ROOT/rltk.js"
