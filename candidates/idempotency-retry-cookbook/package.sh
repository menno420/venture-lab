#!/bin/sh
# package.sh — assemble a clean, buyer-facing bundle of The Idempotency &
# Retry Cookbook into
# candidates/idempotency-retry-cookbook/dist/idempotency-retry-cookbook-v0.1.zip
#
# Buyer bundle = README + QUICKSTART + INCLUDED + PROVENANCE + guide/ +
# recipes/. It deliberately EXCLUDES (allow-list, never a blanket copy):
...
#   - __pycache__   (Python bytecode cruft from running the self-test)
#   - .git          (repo metadata)
#
...
SRC="$SCRIPT_DIR"
BUNDLE_NAME="idempotency-retry-cookbook-v0.1"
DIST_DIR="$SRC/dist"
