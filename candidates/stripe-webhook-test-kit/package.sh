#!/bin/sh
# Build the buyer bundle for Stripe Webhook Test Kit v0.1.
# Bundles: the harness (swtk.py + swtk.js), the example handler, the HTTP-layer
# test suite, the vendored real-shape fixtures + PROVENANCE, README, GOTCHAS.
# EXCLUDES: INTAKE.md (internal), package.sh (build tooling), dist/, .git, __pycache__.
set -eu

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
SRC="$SCRIPT_DIR"
BUNDLE_NAME="stripe-webhook-test-kit-v0.1"
DIST_DIR="$SRC/dist"
