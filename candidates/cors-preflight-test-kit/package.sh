#!/bin/sh
# Build the buyer bundle for CORS Preflight Test Kit v0.1.
# Bundles: the harness (cptk.py + cptk.js), the CORRECT + NAIVE reference
# endpoints, the HTTP-layer test suite, the docs-derived fixtures + MANIFEST +
# PROVENANCE, README, GOTCHAS, QUICKSTART, .env.example.
...
# rate-limit + pagination + JWT kits).
set -eu

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
SRC="$SCRIPT_DIR"
BUNDLE_NAME="cors-preflight-test-kit-v0.1"
DIST_DIR="$SRC/dist"
...
cp "$SRC/cptk.py"                  "$ROOT/cptk.py"
cp "$SRC/cptk.js"                  "$ROOT/cptk.js"
