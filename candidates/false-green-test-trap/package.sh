#!/bin/sh
# package.sh — assemble a clean, buyer-facing bundle of The False-Green
# Test Trap into candidates/false-green-test-trap/dist/false-green-test-trap-v0.1.zip
#
# Buyer bundle = README + QUICKSTART + INCLUDED + vendor_fixture.py +
# test_vendor_fixture.py + guide/ + examples/.
...
#   - __pycache__/  (runtime cruft from local test runs)
#   - .git          (repo metadata)
#
...
SRC="$SCRIPT_DIR"
BUNDLE_NAME="false-green-test-trap-v0.1"
DIST_DIR="$SRC/dist"
...
cp "$SRC/vendor_fixture.py"      "$ROOT/vendor_fixture.py"
cp "$SRC/test_vendor_fixture.py" "$ROOT/test_vendor_fixture.py"

# --- the guide + examples: copy verbatim, then prune non-product cruft ---
