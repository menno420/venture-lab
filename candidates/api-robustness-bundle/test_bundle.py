#!/usr/bin/env python3
"""Assembly / inventory check for the API Robustness Bundle.
...
component dir). This check verifies the BUNDLE is assembled honestly:
  ...
  3. The built bundle zip (dist/api-robustness-bundle-v0.1.zip) contains
...
BUNDLE_ZIP = HERE / "dist" / "api-robustness-bundle-v0.1.zip"
...
class BundleZipAssembled(unittest.TestCase):
    def test_bundle_zip_exists(self):
        self.assertTrue(
            BUNDLE_ZIP.is_file(),
            "dist/api-robustness-bundle-v0.1.zip missing — run ./package.sh",
        )
    ...
    def test_bundle_contains_pinned_component_zips(self):
        with zipfile.ZipFile(BUNDLE_ZIP) as zf:
            ...
            for comp in MANIFEST["components"]:
                inner = f"api-robustness-bundle-v0.1/kits/{comp['dist']}"
    ...
    def test_bundle_contains_docs(self):
        with zipfile.ZipFile(BUNDLE_ZIP) as zf:
            names = set(zf.namelist())
        base = "api-robustness-bundle-v0.1"
    ...
    def test_bundle_holds_no_extra_kits(self):
        with zipfile.ZipFile(BUNDLE_ZIP) as zf:
            packed_kits = {
                ...
                if n.startswith("api-robustness-bundle-v0.1/kits/")
                and n.endswith(".zip")
            }
