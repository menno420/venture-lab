# The Seed Catalogue Courtship — large-print paperback edition spec

Production spec for a large-print paperback of the **full** epistolary
novella (canonical manuscript `../../en/the-seed-catalogue-courtship.md`,
**15,133 words** per
`wc -w candidates/adult-novels/the-seed-catalogue-courtship/en/the-seed-catalogue-courtship.md`
→ `15133`, 2026-07-13). **Spec only — no new manuscript needed**; the
interior is set from the base manuscript unchanged. Structure mirrors the
...
([`docs/publishing/vetting/the-seed-catalogue-courtship.md`](../../../../../docs/publishing/vetting/the-seed-catalogue-courtship.md) §7).
...
- **Epistolary layout is THE typographic decision for this title.** The
  text is built from ~60 bracketed italic document headers (*[Letter, dated
  6 February…]*) plus order forms, packet instructions, and letters. For a
  large-print interior: set the document headers as short set-off lines in
  roman small-size caps or bold — **not** sustained italic (long italics are
  a documented low-vision stumbling block) — and keep the letters themselves
  in the roman body face with a clear one-line space and indent marking each
  document boundary. Never set a whole letter italic.
...
- Part openers: the book is in four parts (Catalogue Weather → Winter
  Work) — start each on a new page; no drop caps.
...
- Body: 15,133 words ÷ 160–200 wpp ≈ **76–95 pages**.
- Plus front/back matter, content note, and 4 part-opener pages: ≈ 6–10
  pages.
- **Epistolary correction:** ~60 document boundaries each cost header +
  whitespace lines, so effective density runs below the band's prose
  assumption — the estimate skews toward (or past) its high end.
...
- **Recommended list: $12.99.** The adult large-print precedents
  ...
  **bottom of the only verified paperback band** ($12.99–$14.99, CHECKLIST
  §4). Yields ~$5.41–$5.49/unit. (Comparable large-print clean-romance
...
- Content note from the manuscript front matter carries over verbatim (adult
  epistolary romance, clean & wholesome: remembered bereavement — a sister, a
  spouse — the winding-up of a family firm, and one badly kept promise; no
  explicit content, no swearing; closed-door throughout).

## Title-specific notes

- Clean/wholesome romance with a sixty-one-year-old heroine and a gardening
  frame is a natural register match for the traditional large-print
  readership — plausibly among the catalog's best fits, but demand is **not
  measured**: no comps surveyed, no sales data exists for this title in any
  format.
- The subtitle *"A Novel in Letters"* stays on the cover (vetting packet §1
  recommends it ON — form-signalling).
...
1. Typeset interior from `../../en/the-seed-catalogue-courtship.md` at the
   spec above — prototype the document-header treatment on one part first;
