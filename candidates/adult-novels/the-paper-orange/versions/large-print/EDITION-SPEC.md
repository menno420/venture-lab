# The Paper Orange — large-print paperback edition spec

Production spec for a large-print paperback of the **full** EN novella
(canonical manuscript `../../en/the-paper-orange.md`, **15,811 words** per
`wc -w candidates/adult-novels/the-paper-orange/en/the-paper-orange.md` →
`15811`, 2026-07-13). **Spec only — no new manuscript needed**; the interior
is set from the base manuscript unchanged. This spec covers the **EN edition
only** — the Dutch edition (`../nl/de-papieren-sinaasappel.md`, 16,203 words
measured) would need its own spec if a large-print NL paperback is ever
wanted. Structure mirrors the merged the-slow-word large-print spec
...
- **Face:** high-legibility book face with open counters and distinct
  ...
  manuscript uses inline italics for Dutch terms (*Oranje boven*, ration-era
  vocabulary) and a few short set-off italic lines (a note, a printed slip) —
...
- Body: 15,811 words ÷ 160–200 wpp ≈ **79–99 pages**.
...
- **Recommended list: $12.99.** The adult large-print precedents
  ...
  **bottom of the only verified paperback band** ($12.99–$14.99, CHECKLIST
  §4). Yields ~$5.41–$5.49/unit. (Comparable large-print
  literary-historical pricing not surveyed — not measured.)
...
- Content note from the manuscript front matter carries over verbatim (adult
  literary-historical fiction, Dutch Hunger Winter 1944–45: wartime famine
  and its effects on children and the elderly, deaths from hunger and cold —
  restrained, largely offpage — bereavement and grief, references to the
  persecution and deportation of Amsterdam's Jewish community, occupation
  raids and one distant shooting; no graphic violence, no explicit content).

## Title-specific notes

- WWII-era historical fiction is a core large-print/library category (the
  readership that lived closest to this history skews oldest) — plausibly the
  catalog's strongest thematic fit for the format, but demand is **not
  measured**: no comps surveyed, no sales data exists for this title in any
  format.
- The subtitle *"A Novel of the Hunger Winter"* stays on the cover (vetting
  packet §1 recommends it ON — the bare title reads ambiguous without it).
...
1. Typeset interior from `../../en/the-paper-orange.md` at the spec above;
