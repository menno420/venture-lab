# The Morning Door (Night Kiln, Book Two) — large-print paperback edition spec

Production spec for a large-print paperback of the **full** novella (canonical
manuscript `../../en/the-morning-door.md`, **15,995 words** per
`wc -w candidates/adult-novels/the-night-kiln/en/the-morning-door.md` →
`15995`, 2026-07-13). **Spec only — no new manuscript needed**; the interior
is set from the base manuscript unchanged. Book One (*The Night Kiln*) is
specced separately in
[`../large-print/EDITION-SPEC.md`](../large-print/EDITION-SPEC.md) — one spec
dir per manuscript. Structure mirrors the merged the-slow-word large-print
spec
...
**Scope note — length-band question untouched.** This spec covers the
manuscript **as written: 15,995 words** (book-1 parity per the ⚑ OWNER length
call recorded in `../../DECISIONS.md`, 2026-07-13). The open owner question —
whether Book Two should be extended toward the vetting packet's ~20k–30k §3
band — is **not touched, not pre-empted, and not answered here**. If the
owner later extends the manuscript, every measured/estimated number below
(word count, page estimate, print cost, royalty rows) must be recomputed from
the new `wc -w` before production.
...
- **Face:** high-legibility book face with open counters and distinct
  ...
  manuscript uses inline emphasis italics plus a few short set-off italic
  lines (the letter from Stonebeck, told memories) — keep each insert short;
  never set whole paragraphs italic.
...
- **Recommended list: $12.99** — matched to the Book One large-print spec
  (series parity in format and price), which itself takes the bottom of the
  only verified paperback band ($12.99–$14.99, CHECKLIST §4) because a 16k
  novella typesets to ~105 pages, half the extent of the ~$14.99 adult
  precedents. Yields ~$5.41–$5.49/unit. (Comparable large-print pricing not
...
- The series' recommended **ebook** price is $4.99 (vetting packet §4) — a
  large-print paperback is structurally ~2.6× the ebook.
...
- Content note from the manuscript front matter carries over verbatim (adult
  cozy fantasy: grief, remembered bereavement — offpage, long in the past —
  and a beloved character living with memory loss, all handled gently; no
  violence, no explicit content; second Night Kiln novella, stands alone).

## Title-specific notes

- **Series consistency:** trim, typography, and price recommendation match
  the Book One spec (`../large-print/`) so the two books sit side by side on
  the same product page in the same format; the "stands alone" line from the
  content note carries to the back cover.
- ⚑ **The owner length-band question stays open** (see the scope note above
  and `../../DECISIONS.md`): this spec neither argues for nor against
  extension — it prices the book that exists.
- Sensible only as a near-zero-marginal-cost catalog extension once a
  standard edition exists; not a lead format. Demand **not measured**.

## Production steps (when owner green-lights)

1. Confirm the ⚑ length-band decision state first: if the owner has ordered
   an extension, stop and respec from the new measured count.
2. Typeset interior from `../../en/the-morning-door.md` at the spec above;
   record actual page count.
3. Recompute the royalty row for the real page count (formula above; check
   which side of the 110-page flat-band boundary it lands).
4. Cover: reuse the standard-edition cover art (when it exists) with a
   "Large Print" banner; spine width from the KDP calculator at final page
   count.
5. Run the full `docs/publishing/CHECKLIST.md` pass.
